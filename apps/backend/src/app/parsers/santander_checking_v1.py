...
def _build_transactions(
    ...
    for index, row in enumerate(raw_rows):
        # Error messages identify a row only by position (index + MM-DD) -- never
        # its description or amounts. They end up in job.failure_reason and the
        # application log, which must not carry statement text (REQ-CLEAN-003).
        where = f"activity row {index} (dated {row.date_token})"

        if (row.credit is None) == (row.debit is None):
            raise ParserError(
                f"{where} has "
                f"{'both an addition and a subtraction' if row.credit is not None else 'no amount'}"
            )
        ...
        if row.balance is not None and row.balance != running:
            raise ParserError(
                f"{where}: the running balance does not follow from the previous row"
            )
...
def parse(pdf_path: Path, extraction: ExtractionResult) -> ParsedStatement:
    ...
    if parsed_credits != summary_credits or parsed_debits != summary_debits:
        which = []
        if parsed_credits != summary_credits:
            which.append("additions")
        if parsed_debits != summary_debits:
            which.append("subtractions")
        raise ParserError(
            f"parsed {' and '.join(which)} total does not match the statement's "
            f"printed summary -- a transaction row was missed or misread"
        )
